...
numOfResends = 9  # resends of the same command to ensure execution
# name of the usb adapter where the schellenberg stick is mounted
usb_adapter = 'ttyACM0'
...
# checks if the provided json command is valid
    ...
    try:
        ...
        if deviceEnumeratorKey in resultMap and deviceCommandKey in resultMap:
            ...
            if command in commandMap.keys():
                ...
                print_line('Invalid command (not in map) received: ' +
                           command, error=True, sd_notify=True)
    except ValueError as err:
        return False
    return bValid

# creates the required string which is sent to the schellenberg usb
...
# callback for a new message received from mqtt
...
def on_message(client, userdata, msg):
    ...
    if bValid:
        ...
        try:
            ...
            print_line('Error setting command to serial port ' +
                       str(e), error=True, sd_notify=True)
    else:
        print_line('Invalid json received, check readme for the appropriate format. Received json: ' +
                   payload, error=True, sd_notify=True)
    pass

# sets up the logic from the configuration file and sets up the mqtt connection
...
def init():
    ...
    if reporting_mode not in ['mqtt-json']:
        print_line('Configuration parameter reporting_mode set to an invalid value',
                   error=True, sd_notify=True)
        sys.exit(1)
    ...
    if reporting_mode in ['mqtt-json']:
        ...
        if reporting_mode == 'mqtt-json':
            mqtt_client.will_set(
                '{}/$announce'.format(base_topic), payload='{}', retain=True)
        elif reporting_mode == 'mqtt-smarthome':
            mqtt_client.will_set(
                '{}/connected'.format(base_topic), payload='0', retain=True)
        ...
        try:
            ...
            mqtt_client.publish(
                '{}/connected'.format(base_topic), payload='1', retain=True)
            mqtt_client.subscribe(base_topic)
            mqtt_client.loop_start()
            sleep(1.0)  # some slack to establish the connection
    sd_notifier.notify('READY=1')
    print_line('Initialization complete, starting MQTT publish loop',
    ...
    while loop_flag == 1:
        ...
        if counter % 10 == 0:
            ...
            mqtt_client.publish(
                '{}/heartbeat'.format(base_topic), payload=timestamp, retain=True)
...
parser.add_argument(
    '--config_dir', help='set directory where config.ini is located', default=sys.path[0])
